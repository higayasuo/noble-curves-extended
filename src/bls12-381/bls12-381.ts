...
// export const createBls12381 = (randomBytes: RandomBytes) => {
//   const curve = {
//     utils: {
...
import { bls, type CurveFn } from '@noble/curves/abstract/bls';
...
// Types
import { isogenyMap } from '@noble/curves/abstract/hash-to-curve';
import type { Fp, Fp12, Fp2, Fp6 } from '@noble/curves/abstract/tower';
import { psiFrobenius, tower12 } from '@noble/curves/abstract/tower';
import {
  mapToCurveSimpleSWU,
  type AffinePoint,
  type ProjPointType,
...
const isogenyMapG2 = isogenyMap(
  ...
  ].map((i) => i.map((pair) => Fp2.fromBigTuple(pair.map(BigInt)))) as [
    Fp2[],
    Fp2[],
    Fp2[],
    Fp2[],
 ...
 */
export const createBls12_381 = (randomBytes: RandomBytes): CurveFn => {
  const bls12_381 = bls({
    // Fields
    fields: {
      Fp,
      ...
      toBytes: (c, point, isCompressed) => {
        ...
        if (isCompressed) {
          ...
          if (isZero) {
            ...
            return concatB(
              numberToBytesBE(x, Fp.BYTES),
              numberToBytesBE(y, Fp.BYTES),
            );
          }
        }
      },
    ...
    G2: {
      ...
      toBytes: (c, point, isCompressed) => {
        ...
        if (isCompressed) {
          ...
          const flag = Boolean(
            y.c1 === _0n ? (y.c0 * _2n) / P : (y.c1 * _2n) / P,
          );
          return concatB(
            setMask(numberToBytesBE(x.c1, len), {
              compressed: true,
              sort: flag,
            }),

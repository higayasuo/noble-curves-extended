...
- Fixed `algorithmToCurveName` to return `undefined` instead of throwing an error when the algorithm cannot uniquely determine a curve name (e.g., EdDSA can map to Ed25519 or Ed448).
- Fixed `resolveCurveName` to not throw a "Curve name mismatch" error when `curveName` is provided and `algorithmName` cannot uniquely determine a curve name (e.g., `curveName: 'Ed25519'` and `algorithmName: 'EdDSA'`). In such cases, the provided `curveName` is returned.

## [0.2.8] - 2025-11-26

### Added

- Added `resolveCurveName` utility function to resolve curve names from either a curve name or an algorithm name. If both are provided, validates that they are consistent.
- Added `algorithmToCurveName` utility function to convert algorithm names (ES256, ES384, ES512, ES256K) to their corresponding curve names.
- Added comprehensive tests for `resolveCurveName` utility functions in `resolveCurveName.spec.ts`.
- Updated README with documentation for curve name resolution utilities.
